...
# Помечен как ожидаемо провалившийся, потому что в коде есть известная ошибка. Результат: XFAIL
@pytest.mark.xfail(reason='Найден баг в приложении, из-за которого тест падает с ошибкой')
def test_with_bug():
    assert 1 == 2


# Помечен как ожидаемо провалившийся, но так как тест проходит успешно, то результат: XPASS
